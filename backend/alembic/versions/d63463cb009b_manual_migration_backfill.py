...
def upgrade() -> None:
    ...
    if has_table:
        ...
        if "source_type" not in columns:
            op.add_column(
                "inspections",
                sa.Column(
                    "source_type", sa.String(length=50), server_default="CSV_IMPORT", nullable=True
                ),
            )
        ...
        if "ix_inspections_processing_status" not in indexes:
            op.create_index(
                "ix_inspections_processing_status",
                "inspections",
                ["processing_status"],
                unique=False,
            )
    ...
    # Perform the updates
    op.execute(
        "UPDATE inspections SET processing_status = 'completed' WHERE inspector_name IS NOT NULL AND processing_status IS NULL;"
    )
    op.execute(
        "UPDATE inspections SET processing_status = 'pending' WHERE processing_status IS NULL;"
    )
